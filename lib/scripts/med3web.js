...
export default class Med3Web {
  ...
  */
  performSkullRemove() {
    ...
    // check correct texture pixel format
    const tformat = tex3d.textureFormat;
    if (tformat !== BABYLON.Engine.TEXTUREFORMAT_R) {
      console.log(`ktxOnSuccess. texture format should be TEXTUREFORMAT_R, but = ${tformat}`);
    }
    //  BABYLON.Engine.TEXTUREFORMAT_RGBA) {
    const textype = tex3d.textureType;
    if (textype !== BABYLON.Engine.TEXTURETYPE_UNSIGNED_BYTE) {
      console.log(`ktxOnSuccess. textype should be TEXTURETYPE_UNSIGNED_BYTE, but = ${textype}`);
    }
    const bytes = dataArray;
    const BPP = 1;
    this.m_engine2d.create3dTexture(BPP, bytes, xDim, yDim, zDim, false);
  ...
  _initScene2d() {
    if (this.m_container2d === null) {
      console.log('this.m_container2d === null !!!');
      return false;
    }
    if (this.m_containerMpr == null) {
      console.log('this.m_containerMpr === null !!!');
      return false;
    }
    const w2d = Math.floor(this.m_container2d.width());
    const h2d = Math.floor(this.m_container2d.height());
    // const canvas = this.m_glcheck.m_staticCanvas;
    ...
    // mouse callback
    const canvas2d = this.m_engine2d.m_canvas2d;
    const canvasMpr = this.m_engineMpr.m_canvasMpr;

    canvas2d.addEventListener('pointerdown', (evt) => {
      // console.log(`pointerdown event. event pos = ${evt.offsetX}, ${evt.offsetY}`);
    ...
    canvas2d.addEventListener('pointermove', (evt) => {
      // console.log(`pointermove event. event pos = ${evt.offsetX}, ${evt.offsetY}`);
      if (this.m_renderMode === RenderMode.RENDER_MODE_2D) {
        const x = evt.offsetX;
        const y = evt.offsetY;
        const xScr = x / w2d;
    ...
    canvas2d.addEventListener('pointerup', (evt) => {
      // console.log(`pointerup event. event pos = ${evt.offsetX}, ${evt.offsetY}`);
      if (this.m_renderMode === RenderMode.RENDER_MODE_2D) {
        const x = evt.offsetX;
        const y = evt.offsetY;
        const xScr = x / w2d;
    ...
    canvas2d.addEventListener('pointerwheel', (evt) => {
      // console.log(`pointerwheel event. event pos = ${evt.originalEvent.deltaY}`);
      if (this.m_renderMode === RenderMode.RENDER_MODE_2D) {
        const wheelDeltaY = evt.deltaY;
        this.m_engine2d.onMouseWheel(-wheelDeltaY);
      }
    });
    canvas2d.addEventListener('wheel', (evt) => {
      // console.log(`wheel event. event delta = ${evt.deltaY}`);
      if (this.m_renderMode === RenderMode.RENDER_MODE_2D) {
    ...
    canvasMpr.addEventListener('pointerdown', (evt) => {
      // console.log(`pointerdown event. event pos = ${evt.offsetX}, ${evt.offsetY}`);
      if (this.m_renderMode === RenderMode.RENDER_MODE_MPR) {
        const x = evt.offsetX;
        const y = evt.offsetY;
        const xScr = x / wMpr;
        const yScr = y / hMpr;
    ...
    canvasMpr.addEventListener('pointermove', (evt) => {
      // console.log(`pointerdown event. event pos = ${evt.offsetX}, ${evt.offsetY}`);
      if (this.m_renderMode === RenderMode.RENDER_MODE_MPR) {
        const x = evt.offsetX;
        const y = evt.offsetY;
    ...
    canvasMpr.addEventListener('pointerup', (evt) => {
      // console.log(`pointerdown event. event pos = ${evt.offsetX}, ${evt.offsetY}`);
      if (this.m_renderMode === RenderMode.RENDER_MODE_MPR) {
        const x = evt.offsetX;
        const y = evt.offsetY;
  ...
  _initScene3d() {
    const NEED_TO_CHECK_OPENGL = false;
    const DEFAULT_TEXTURE_SIZE = 4096;
    let maxTextureSize = DEFAULT_TEXTURE_SIZE;
    if (NEED_TO_CHECK_OPENGL) {
      // check web gl
      const glcheck = this.m_glcheck;
      ...
      maxTextureSize = glcheck.getMaxTextureSize();
      ...
      if (supportedFlags !== GlCheck.SUPPORT_ALL) {
        ...
        console.log(`GlCheck. not good supported flags. msg = ${strLog}`);
        return false;
      } // if not all flags are supported
    } // if need to chwkc openGL
  ...
  */
  ...
  _dicomOnSucces(dataName, loader, header, dataSize, dataArray) {
    ...
    if (this.m_engine2d.m_volumeData === null) {
      ...
      // check correct texture pixel format
      const tex3d = this.m_engine3d.bVolume3D;
      const tformat = tex3d.textureFormat;
      if (tformat !== BABYLON.Engine.TEXTUREFORMAT_R) {
        console.log(`ktxOnSuccess. texture format should be TEXTUREFORMAT_R, but = ${tformat}`);
      }
      ...
      const bytes = dataArray;
      this.m_engine2d.setVolumeBox(box);
      this.m_engineMpr.setVolumeBox(box);
      const BPP = 1;
      this.m_engine2d.create3dTexture(BPP, bytes, xDim, yDim, zDim, false);
      this._callbackFileLoaded();
    }
  ...
  _ktxOnSuccess(dataName, loader, header, dataSize, dataArray) {
    ...
    if (this.m_engine2d.m_volumeData === null) {
      ...
      // this.m_engine3d.origVolumeTex is null.
      // for (let prop in this.m_engine3d) {
      //   console.log(`obj[${prop}] = ${this.m_engine3d[prop]}`);
      // }

      // BABYLON.RawTexture3D
      const tex3d = this.m_engine3d.bVolume3D;
      // tex3d.is3D should be true
      const is3d = tex3d.is3D;
      if (!tex3d.is3D) {
        console.log(`ktxOnSuccess. is 3d wrong. 3d = ${is3d}`);
      }
  ...
  _hdrOnSuccess(dataName, loader, header, dataSize, dataArray, isRoiVolume) {
    ...
    if (this.m_engine2d.m_volumeData === null) {
      ...
      const is3d = tex3d.is3D;
      if (!tex3d.is3D) {
        console.log(`hdrOnSuccess. is 3d wrong. 3d = ${is3d}`);
      }
      ...
      if (tformat === BABYLON.Engine.TEXTUREFORMAT_RGBA) {
        bpp = FOUR;
        // console.log('HdrOnSuccess. Tex format is 4 bytes');
      }
      if (bpp === BAD) {
        console.log(`hdrOnSuccess. text format expect TEXTUREFORMAT_R or TEXTUREFORMAT_RGBA, but = ${tformat}`);
      }
      //  BABYLON.Engine.TEXTUREFORMAT_RGBA) {
      const textype = tex3d.textureType;
      if (textype !== BABYLON.Engine.TEXTURETYPE_UNSIGNED_BYTE) {
        console.log(`hdrOnSuccess. textype should be TEXTURETYPE_UNSIGNED_BYTE, but = ${textype}`);
      }
      const xDim = header.m_pixelWidth;
      const yDim = header.m_pixelHeight;
      const zDim = header.m_pixelDepth;
      ...
      this.m_engine2d.setVolumeBox(box);
      this.m_engineMpr.setVolumeBox(box);
      this.m_engine2d.create3dTexture(bpp, bytes, xDim, yDim, zDim, isRoiVolume);
      this._callbackFileLoaded();
    }
  }
