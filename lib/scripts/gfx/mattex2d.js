...
export default class MaterialTex2d {
  ...
  constructor(scene) {
    ...
    const srcShaderVertex = 'precision highp float;\n\
// Attributes \n\
...
void main(void) { \n\
    ...
    float xZoom = uv.x * zoom + posX; \n\
    float yZoom = uv.y * zoom + posY; \n\
    if (plane == SLICE_AXIS_X)\n\
      vUV = vec3(sliceValue, xZoom, yZoom); \n\
    if (plane == SLICE_AXIS_Y)\n\
      vUV = vec3(xZoom, sliceValue, yZoom); \n\
    if (plane == SLICE_AXIS_Z)\n\
      vUV = vec3(xZoom, yZoom, sliceValue); \n\
}';

    // shader source pixel
    // eslint-disable-next-line
    const srcShaderFragment = 'precision highp float;\n\
...
void main(void) { \n\
    if (textureReady != 0) { \n\
      vec4 vColTex = texture(texture3D, vUV); \n\
      vec4 col = vec4(vColTex.x, vColTex.x, vColTex.x, 1.0); \n\
      if (isRoiVolume != 0) { \n\
        if (vColTex.a > 0.4) { \n\
    ...
    const SHADER_NAME_VERT = 'mattex2dVertexShader';
    const SHADER_NAME_FRAG = 'mattex2dFragmentShader';
    BABYLON.Effect.ShadersStore[SHADER_NAME_VERT] = srcShaderVertex;
    BABYLON.Effect.ShadersStore[SHADER_NAME_FRAG] = srcShaderFragment;

    // create material
    const MAT_NAME_2D = 'mattex2d';
    this.m_bMaterial = new BABYLON.ShaderMaterial(MAT_NAME_2D, scene, {
      vertex: MAT_NAME_2D,
      fragment: MAT_NAME_2D
    }, {
      attributes: ['position', 'uv'],
      uniforms: ['worldViewProjection', 'sliceValue', 'textureReady', 'plane', 'isRoiVolume', 'zoom', 'posX', 'posY']
    });
    this.m_bMaterial.setInt('textureReady', 0);
    const Z_LEVEL_DEFAULT = 0.5;
    this.m_bMaterial.setFloat('sliceValue', Z_LEVEL_DEFAULT);
    this.m_bMaterial.setInt('plane', TEX2D_PLANE_Z);
    this.m_bMaterial.setFloat('zoom', 1.0);
    this.m_bMaterial.setFloat('posX', 0.0);
    this.m_bMaterial.setFloat('posY', 0.0);
    this.m_bMaterial.setInt('isRoiVolume', 0);
  }
  getBabylonMaterial() {
    return this.m_bMaterial;
  }
