...
/**
* Orbit control
* @module lib/scripts/controls/babylonorbitcontrol
*/
...
export default class BOrbitControl {

  /**
  * Initialize render
  * @return {Object} Intsance of this class (singleton)
  */
  constructor(objContainer, camera, mesh, meshRotationCallback) {
    this.m_callback = meshRotationCallback;
    this.m_mesh = mesh;
    this.m_camera = camera;
    this.m_target = new BABYLON.Vector3(0, 0, 0);
    this.m_button = BOrbitControl.EVENT_BUTTON_NA;
  ...
  updateTime(dx, dy) {
    ...
    // console.log(`mouse move event at ${dx}, ${dy}`);
    if (!this.m_mesh) {
      return;
    }

    //const matrix = new BABYLON.Matrix.Identity();
    const camDir = this.m_target.subtract(this.m_camera.position);
    camDir.normalize();
    //camDir.clone(this.m_target).subtract(this.m_camera.position).normalize();
    //camDir.clone(this.m_target).subtract(this.m_camera.position).normalize();
    //const rotationX = BABYLON.Matrix.RotationAxis(this.m_camera.upVector, dx * CONTROL_CAMERA_ROTATION_SPEED);
    //const rotationY = BABYLON.Matrix.RotationAxis(BABYLON.Vector3.Cross(camDir, this.m_camera.upVector), dy * CONTROL_CAMERA_ROTATION_SPEED);
    //matrix.multiply(rotationX).multiply(rotationY).multiply(this.m_mesh.worldMatrixFromCache);

      //this.m_mesh.rotation.setFromRotationMatrix(matrix);
      //matrix.decompose({rotation: this.m_mesh.rotationQuaternion});
    const rotationX = BABYLON.Quaternion.RotationAxis(this.m_camera.upVector, dx * CONTROL_CAMERA_ROTATION_SPEED);
    const rotationY = BABYLON.Quaternion.RotationAxis(BABYLON.Vector3.Cross(camDir, this.m_camera.upVector), dy * CONTROL_CAMERA_ROTATION_SPEED);
    //this.m_mesh.rotation = rotationY.multiply(this.m_mesh.rotation);
    //this.m_mesh.rotation = rotationX.multiply(this.m_mesh.rotation);
  ...
  onMouseMove(x, y) {
    ...
    this.m_prevMouse.x = x;
    this.m_prevMouse.y = y;
    //console.log(`2mouse move event at ${dx}, ${dy}`);
  ...
  addCallbacks() {
    this.m_container.on('mousedown', (event) => {
      ...
      if (buttonIndex === BOrbitControl.EVENT_BUTTON_LEFT) {
  ...
  onZoom(delta) {
    ...
    const camDir = this.m_camera.position.subtract(this.m_target);
...
BOrbitControl.EVENT_BUTTON_NA = -1;
BOrbitControl.EVENT_BUTTON_LEFT = 0;
BOrbitControl.EVENT_BUTTON_CENTER = 1;
BOrbitControl.EVENT_BUTTON_RIGHT = 2;
