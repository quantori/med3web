...
export default class MprRenderer {
  ...
  */
  create() {
    ...
    this.m_bCamera = new BABYLON.FreeCamera(CAM_NAME, new BABYLON.Vector3(0, 0, CAM_Z_POS), this.m_bScene);
    ...
    const matGreen = new MaterialColor2d(R_MATERIAL_GREEN, G_MATERIAL_GREEN, B_MATERIAL_GREEN, this.m_bScene);
    this.m_materialLineGreen = matGreen.m_bMaterial;
    const R_MATERIAL_BLUE = 0.0;
    const G_MATERIAL_BLUE = 0.8125;
    const B_MATERIAL_BLUE = 0.8125;
    const matBlue = new MaterialColor2d(R_MATERIAL_BLUE, G_MATERIAL_BLUE, B_MATERIAL_BLUE, this.m_bScene);
    this.m_materialLineBlue = matBlue.m_bMaterial;

    this.m_bMaterial = new Array(NUM_PROJECTIONS);
    this.m_bMeshes = new Array(NUM_PROJECTIONS);
    this.m_vertexData = new Array(NUM_PROJECTIONS);
    let i;
    for (i = 0; i < NUM_PROJECTIONS; i++) {
      this.m_bMaterial[i] = null;
      this.m_bMeshes[i] = null;
      this.m_vertexData[i] = null;
    }
    ...
    for (i = 0; i < NUM_BOUND_LINES; i++) {
      this.m_boundLines[i] = null;
    }
  } // create

  clearScene() {
    for (let i = 0; i < this.m_boundLines.length; ++i) {
      if (this.m_boundLines[i] !== null) {
        this.m_boundLines[i].m_bMesh.dispose();
      }
    }
    for (let i = 0; i < this.m_bMeshes.length; ++i) {
      if (this.m_bMeshes[i] !== null) {
        this.m_bMeshes[i].dispose();
      }
    }
  ...
  */
  onFileLoaded() {
    ...
    console.log('Mpr.OnFileLoaded');
    this.clearScene();

    // create 3d texture from bytes array
    ...
    const arrBytesSrc = this.m_objGraphics2d.m_arrBytes;
    const bpp = this.m_objGraphics2d.m_bpp;
    const numPixels = xDim * yDim * zDim;
    if (arrBytesSrc.length !== numPixels * bpp) {
      console.log('Mpr. onFileLoaded. Bad texture format. should be 1 or 4 bpp');
    }
    // create texture from bytes: one per each projection
    ...
    this.m_volTexture3d = new BABYLON.RawTexture3D(arrBytesSrc,
      xDim, yDim, zDim,
      texFormat, this.m_bScene,
      GEN_MIPMAPS, INVERT_Y, SAMPLING_MODE, TEX_TYPE);
    // console.log(`Mpr. Create copy of 3d texture: ${xDim}*${yDim}*${zDim}`);
    ...
    // TODO: only 1 projects use for test
    // for (i = 0; i < 1; i++) {
    for (i = 0; i < NUM_PROJECTIONS; i++) {
      let wPhys = 0.0;
      let hPhys = 0.0;
      ...
      this.m_bMeshes[i].position = new BABYLON.Vector3(0, 0, 0);
      ...
      // v0 ----- v1
      const positions = [];
      positions.push(XMin, YMin, Z_PLANE);
      positions.push(XMax, YMin, Z_PLANE);
      positions.push(XMax, YMax, Z_PLANE);
      positions.push(XMin, YMax, Z_PLANE);
      // console.log(`Mpr. quad = ${positions[0]}, ${positions[1]},${positions[2]}`);
      // console.log(`Mpr. quad = ${positions[3]}, ${positions[4]},${positions[5]}`);
      // console.log(`Mpr. quad = ${positions[6]}, ${positions[7]},${positions[8]}`);
      // console.log(`Mpr. quad = ${positions[9]}, ${positions[10]},${positions[11]}`);
      ...
      this.m_vertexData[i] = new BABYLON.VertexData();
      this.m_vertexData[i].positions = positions;
      this.m_vertexData[i].normals = normals;
      this.m_vertexData[i].indices = indices;
      this.m_vertexData[i].uvs = uvCoords;
      this.m_vertexData[i].colors = null;
      this.m_vertexData[i].applyToMesh(this.m_bMeshes[i]);

      // create material
      const matTex = new MaterialTex2d(this.m_bScene);
      const mat = matTex.getBabylonMaterial();

      // set babylon material props
      mat.setTexture('texture3D', this.m_volTexture3d);
      const IS_TEXTURE_READY = 1;
      mat.setInt('textureReady', IS_TEXTURE_READY);
      mat.setInt('isRoiVolume', 0);
      mat.setFloat('posX', 0.0);
      mat.setFloat('posY', 0.0);
      mat.setFloat('zoom', 1.0);
      mat.setFloat('sliceValue', this.m_sliceRatio[i]);
      // eslint-disable-next-line
      const axisIndex = 2 - i; // inside matTex2d.create it should be X - 0 or Y - 1 or Z - 2
      mat.setInt('plane', axisIndex);
      this.m_bMaterial[i] = mat;
      this.m_bMeshes[i].material = mat;

      const TEST_COLORED_MAT = false;
      if (TEST_COLORED_MAT) {
        const R_COL = i / NUM_PROJECTIONS;
        const G_COL = 0.1;
        const B_COL = 0.9;
        const matCol = new MaterialColor2d(R_COL, G_COL, B_COL, this.m_bScene);
        const matBab = matCol.m_bMaterial;
        this.m_bMeshes[i].material = matBab;
      }
    ...
    const matLine = new MaterialColor2d(R_MATERIAL, G_MATERIAL, B_MATERIAL, this.m_bScene);
    const mat = matLine.m_bMaterial;
    const minCoord = (-1.0 / TWO) + this.m_vertLineWidth;
    const maxCoord = (+1.0 / TWO) - this.m_vertLineWidth;
    const halfLineWidth = this.m_vertLineWidth * 0.5;
    this.m_boundLines[0] = new Line2D(this.m_bScene, this.m_horLineWidth, minCoord, maxCoord, maxCoord, maxCoord, mat);
    this.m_boundLines[1] = new Line2D(this.m_bScene, this.m_vertLineWidth, maxCoord, maxCoord, maxCoord, minCoord, mat);
    this.m_boundLines[2] = new Line2D(this.m_bScene, this.m_horLineWidth, maxCoord, minCoord, minCoord, minCoord, mat);
    this.m_boundLines[3] = new Line2D(this.m_bScene, this.m_vertLineWidth, minCoord, minCoord, minCoord, maxCoord, mat);
    this.m_boundLines[4] = new Line2D(this.m_bScene, this.m_vertLineWidth,
      this.m_projectionRect[0].xMax - halfLineWidth,
      minCoord, this.m_projectionRect[0].xMax - halfLineWidth, maxCoord, mat);
    this.m_boundLines[5] = new Line2D(this.m_bScene, this.m_vertLineWidth,
  ...
  clearControlLines() {
    if (this.m_linesXOnZ !== null) {
      this.m_linesXOnZ[0].m_bMesh.dispose();
      this.m_linesXOnZ[1].m_bMesh.dispose();
    }
    if (this.m_linesYOnZ !== null) {
      this.m_linesYOnZ[0].m_bMesh.dispose();
      this.m_linesYOnZ[1].m_bMesh.dispose();
    }
    if (this.m_linesXOnY !== null) {
      this.m_linesXOnY[0].m_bMesh.dispose();
      this.m_linesXOnY[1].m_bMesh.dispose();
    }
    if (this.m_linesYOnX !== null) {
      this.m_linesYOnX[0].m_bMesh.dispose();
      this.m_linesYOnX[1].m_bMesh.dispose();
    }
    if (this.m_controlLineXOnY !== null) {
      this.m_controlLineXOnY.m_bMesh.dispose();
    }
    if (this.m_controlLineYOnX !== null) {
      this.m_controlLineYOnX.m_bMesh.dispose();
    }
  }
  ...
  updateControlLines() {
    ...
    this.m_linesXOnZ = [
      new Line2D(this.m_bScene, this.m_vertLineWidth, xCenterOnZ,
        this.m_projectionRect[PROJECTION_Z].yMax, xCenterOnZ, yCenterOnZ + vertShift, this.m_materialLineBlue),
      new Line2D(this.m_bScene, this.m_vertLineWidth, xCenterOnZ, yCenterOnZ - vertShift, xCenterOnZ,
        this.m_projectionRect[PROJECTION_Z].yMin, this.m_materialLineBlue)];
    // green Y projection line on Z projection
    this.m_linesYOnZ = [
      new Line2D(this.m_bScene, this.m_horLineWidth, this.m_projectionRect[PROJECTION_Z].xMin,
        yCenterOnZ, xCenterOnZ - horShift, yCenterOnZ, this.m_materialLineGreen),
      new Line2D(this.m_bScene, this.m_horLineWidth, xCenterOnZ + horShift, yCenterOnZ,
    ...
    this.m_linesXOnY = [
      new Line2D(this.m_bScene, this.m_vertLineWidth, xCenterOnY,
        this.m_projectionRect[PROJECTION_Y].yMax, xCenterOnY, yCenterOnY + vertShift, this.m_materialLineBlue),
      new Line2D(this.m_bScene, this.m_vertLineWidth, xCenterOnY, yCenterOnY - vertShift, xCenterOnY,
        this.m_projectionRect[PROJECTION_Y].yMin, this.m_materialLineBlue)];
    this.m_controlLineXOnY = new Line2D(this.m_bScene, this.m_vertLineWidth, xCenterOnY,
    ...
    this.m_linesYOnX = [
      new Line2D(this.m_bScene, this.m_vertLineWidth, xCenterOnX,
        this.m_projectionRect[PROJECTION_X].yMax, xCenterOnX, yCenterOnX + vertShift, this.m_materialLineGreen),
      new Line2D(this.m_bScene, this.m_vertLineWidth, xCenterOnX, yCenterOnX - vertShift, xCenterOnX,
        this.m_projectionRect[PROJECTION_X].yMin, this.m_materialLineGreen)];
    this.m_controlLineYOnX = new Line2D(this.m_bScene, this.m_vertLineWidth, xCenterOnX,
  ...
  */
  render() {
    ...
    if (this.m_bMaterial[PROJECTION_X] !== null) {
      this.m_bMaterial[PROJECTION_X].setFloat('sliceValue', this.m_sliceRatio[PROJECTION_X]);
    }
    if (this.m_bMaterial[PROJECTION_Y] !== null) {
      this.m_bMaterial[PROJECTION_Y].setFloat('sliceValue', this.m_sliceRatio[PROJECTION_Y]);
    }
    if (this.m_bMaterial[PROJECTION_Z] !== null) {
      this.m_bMaterial[PROJECTION_Z].setFloat('sliceValue', this.m_sliceRatio[PROJECTION_Z]);
    }
  }
