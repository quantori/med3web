  ...
  */
  onFileLoaded() {
    ...
    for (i = 0; i < NUM_PROJECTIONS; i++) {
      ...
      let wScreen = this.m_width;
      let hScreen = this.m_height;
      let wPrjS = wScreen;
      ...
      // const volTexture3d = this.m_objGraphics2d.m_volTexture;
      this.m_bMaterial[i].setTexture('texture3D', volTexture3d);
      const IS_TEXTURE_READY = 1;
      this.m_bMaterial[i].setInt('textureReady', IS_TEXTURE_READY);
      this.m_bMaterial[i].setFloat('posX', 0.0);
      this.m_bMaterial[i].setFloat('posY', 0.0);
      this.m_bMaterial[i].setFloat('posZ', 0.0);
      this.m_bMaterial[i].setFloat('zoom', 1.0);
      this.m_bMaterial[i].setFloat('sliceValue', this.m_sliceRatio[i]);
      // eslint-disable-next-line
      const axisIndex = 2 - i; // inside matTex2d.create it should be X - 0 or Y - 1 or Z - 2
      this.m_bMaterial[i].setInt('plane', axisIndex);
  ...
  clearControlLines() {
    if (this.m_linesXOnZ !== null) {
      this.m_linesXOnZ[0].m_bMesg.dispose();
      this.m_linesXOnZ[1].m_bMesg.dispose();
    }
