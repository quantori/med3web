...
export default class DistanceTool {
   ...
   */
  constructor(scene, lineWidth, textCanvas, textContext) {
    ...
    this.m_linesMaterial = new MaterialColor2d(R_MATERIAL, G_MATERIAL, B_MATERIAL, this.m_bScene);
  ...
  clearLines() {
    ...
    for (let i = 0; i < length; ++i) {
      ...
      // this.m_bScene.remove(dist.line.getRenderObject());
      // this.m_bScene.remove(dist.text);
      dist.line.m_bMesh.dispose();
      dist.text.m_bMesh.dispose();
    }
    this.m_runningState = false;
    this.m_vertexes = [];
  }
  ...
  updateLines(zoom, posX, posY) {
    ...
    const length = this.m_distances.length;
    for (let i = 0; i < length; ++i) {
      // this.m_bScene.remove(this.m_distances[i].line.getRenderObject());
      this.m_distances[i].line.m_bMesh.dispose();
      this.m_distances[i].text.m_bMesh.dispose();

      // this.m_bScene.remove(this.m_distances[i].text);
      ...
      const line = new Line2D(this.m_bScene, this.m_lineWidth, xS, yS, xE, yE, this.m_linesMaterial);
      this.m_distances[i].line = line;
      this.m_distances[i].text.updateText(0.5 * (xS + xE) - 0.00, 0.5 * (yS + yE) - 0.00, this.m_textWidthScr,
        MeshText2D.ALIGN_CENTER, MeshText2D.ALIGN_CENTER, this.m_textBgColor, this.m_textColor, this.m_bScene);
      // this.m_bScene.add(this.m_distances[i].text);
    }
  }
  ...
  onMouseDown(x, y, zoom, posX, posY) {
    ...
    if (this.m_runningState) {
      ...
      const line = new Line2D(this.m_bScene, this.m_lineWidth, x, y, x, y, this.m_linesMaterial);
      const strMsg = '0 mm';
      const text = new MeshText2D(strMsg, this.m_textCanvas, this.m_textContext);
      text.updateText(x, y, this.m_textWidthScr, MeshText2D.ALIGN_CENTER,
        MeshText2D.ALIGN_CENTER, this.m_textBgColor, this.m_textColor, this.m_bScene);
      // this.m_bScene.add(text);
  ...
  onMouseMove(x, y, zoom) {
    if (this.m_runningState) {
      ...
      // remove from scene: new style - via dispose
      // this.m_bScene.remove(dist.line.getRenderObject());
      const lineToDel = dist.line;
      if (!lineToDel.isValid()) {
        console.log('This is not line2d !!!');
      }
      lineToDel.m_bMesh.dispose();

      const line = new Line2D(this.m_bScene, this.m_lineWidth, this.m_xStart, this.m_yStart,
        x, y, this.m_linesMaterial);
      // this.m_bScene.remove(dist.text);
      dist.text.m_bMesh.dispose();
  ...
  updateVertexes(zoom, posX, posY) {
    ...
    for (let i = 0; i < this.m_distances.length; ++i) {
      let xZ = (this.m_distances[i].line2d.getxS() + (1 - 1 / zoom)) * zoom + posX;
      let yZ = (this.m_distances[i].line2d.getyS() - (1 - 1 / zoom)) * zoom + posY;
      this.m_vertexes.push({ xZ, yZ });
      xZ = (this.m_distances[i].line2d.getxE() + (1 - 1 / zoom)) * zoom + posX;
      yZ = (this.m_distances[i].line2d.getyE() - (1 - 1 / zoom)) * zoom + posY;
      this.m_vertexes.push({ xZ, yZ });
    }
  }
}
