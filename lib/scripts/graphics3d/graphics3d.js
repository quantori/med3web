...
export default class Graphics3d {
  createBabylonScene(canvas) {
    ...
    this.bCamera = new BABYLON.UniversalCamera("UniversalCamera", new BABYLON.Vector3(0, 0, 1.5), scene);
    ...
    const bfRenderTexture = new BABYLON.RenderTargetTexture("backface", 1024,
    ...
    //bfRenderTexture.onAfterRender = function () {
    //  bfRenderTexture.renderList[0].overrideMaterialSideOrientation = BABYLON.Mesh.FRONTSIDE;
    //  bfRenderTexture.renderList[0].material = vrMat;
    //}
    ///////////////////////////////////////////////////////////////////////////////
    // Configuring the second rendering pipeline stage: frontface pass
    const ffRenderTexture = new BABYLON.RenderTargetTexture("frontface", 1024,
      //{width: this.windowWidth, height: this.windowHeight},
        scene);
  ...
  startThreeJSRender(engine2d, root3dContainer, curFileDataType) {
    ...
    this.renderer = new THREE.WebGLRenderer({ /*canvas: this.canvas3d, */antialias: false, preserveDrawingBuffer: true });
  ...
  */
  ...
  /**
   * Compute 3D texture coordinates on BBOX. Implement via BABYLON!
  ...
  callbackCreateCubeVolumeBF(window, box, nonEmptyBoxMin, nonEmptyBoxMax, isRoiVolume = false) {
    ...
    this.bVolume3D = new BABYLON.RawTexture3D(
      this.engine2d.m_volumeData,
      xDim,
      yDim,
      zDim,
      volume3dTexFormat,
